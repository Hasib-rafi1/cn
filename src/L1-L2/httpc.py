...
def multithread():
    global thread_count
    getthreads=int(input("Enter number of threads: "))
    getthreadlist=[]
    ...
    for a in getthreadlist:
        ...
        if('get' in split_list):
            ...
            thread_count = thread_count+1
            Thread(target=GET, args=(data["host"],data["param"],data["usage"],data["header"],data["filename"],data["local_param"])).start()


        elif('post' in split_list):
            thread_count = thread_count+1
            data = preprocess_command(a)
            Thread(target=POST, args=(data["host"],data["param"],data["usage"],data["header"],data["filename"],data["json"],data["local_param"],data["overwrite"])).start()
...
def extract_html_objects(a):
    match = a.find('<', 0)
    a[match:]
def GET(host,param,usage,header,filename,local_param):
    hostfinal = host
    port = 80
    if "localhost" in host:
    ...
    if(usage is True):
        ...
        flag = True
        if(filename==''):
            for result in extract_json_objects(a):
                json_formatted_str = json.dumps(result, indent=2)
                if(json_formatted_str=="{}"):
                    b = extract_html_objects(a)
                    print(b)
                    flag = False
                else:
                    print(json_formatted_str)
                    flag = False
            if "localhost" in host and flag == True:
                print(a)
        else:
            f=open(filename,"w")
            for result in extract_json_objects(a):
    ...
    if( a.__contains__('HTTP/1.0 301') or a.__contains__('HTTP/1.0 302') or a.__contains__('HTTP/1.0 303') or a.__contains__('HTTP/1.0 304')):
        host=(re.search('Location: http://(.*)\n',a).group(1))
        host=host.strip()
        host=host.replace('/','')
        GET(host,param,usage,header,filename,local_param)
    print("\n")
    thread_reduce()
def POST(host,param,usage,header,filename,json_data,local_param,overwrite):
    hostfinal = host
    port = 80
    if "localhost" in host:
        url_split=host.split(":")
...
def init():
    ...
    if('help' in split_list):
        ...
        GET(data["host"],data["param"],data["usage"],data["header"],data["filename"],data["local_param"])
        init()
    elif('post' in split_list):
        data = preprocess_command(user_input)
        POST(data["host"],data["param"],data["usage"],data["header"],data["filename"],data["json"],data["local_param"],data["overwrite"])
        init()
init()
